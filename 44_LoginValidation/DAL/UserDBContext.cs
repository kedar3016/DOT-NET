...
namespace _44_LoginValidation.DAL
{
    internal class UserDBContext
    {
        ...
        internal int addNewUser(MyUser user)
        {
            SqlConnection conn = new SqlConnection(ConnectionString);

            SqlCommand cmd = new SqlCommand($"insert into [user] values('{user.userName}','{user.password}','{user.email}')", conn);
        ...
        internal int updatePass(MyUser user)
        {
            SqlConnection conn = new SqlConnection(ConnectionString);

            SqlCommand cmd = new SqlCommand($"update [user] set password ='{user.password}' where email = '{user.email}'", conn);
